"""SNF Cost Reports (HCRIS) pipeline.

Source: CMS HCRIS SNF (~15K reports/year)
Same HCRIS structure as hospital cost reports but with SNF-specific worksheets.
Outputs:
  - data/processed/cost_reports_snf/{data_year}/cost_reports_snf.parquet
  - staging.stg_cms__cost_reports_snf (PostgreSQL)
"""
...
log = get_logger(source="cost_reports_snf")

# SNF-specific metrics from HCRIS worksheets
SNF_FINANCIAL_METRICS = {
    ...
    ("S300001", "0014", "0060"): "total_patient_days",
...
def validate_snf_cr(df: pd.DataFrame) -> ValidationReport:
    report = ValidationReport(source="cost_reports_snf")
...
def run(
    ...
    log.info("cost_reports_snf_start", run_date=str(run_date), data_year=data_year)

    if source_path:
        landing = source_path if source_path.is_dir() else source_path.parent
    else:
        source_def = get_source("cost_reports_snf")
        landing = resolve_landing_path("cost_reports_snf", run_date, data_year)
    ...
    parquet_path = (
        ...
        / "cost_reports_snf" / str(data_year) / "cost_reports_snf.parquet"
    )
    write_parquet(rpt_df, parquet_path)
    results["cost_reports_snf_parquet"] = len(rpt_df)

    out_cols = [c for c in STAGING_COLUMNS if c in rpt_df.columns]
    rows = copy_dataframe_to_pg(rpt_df[out_cols], "stg_cms__cost_reports_snf", "staging", if_exists="append")
    results["stg_cost_reports_snf"] = rows

    log.info("cost_reports_snf_complete", **results)
    return results
