"""HHA Cost Reports (HCRIS) pipeline.

Source: CMS HCRIS HHA (~10K reports/year)
Outputs:
  - data/processed/cost_reports_hha/{data_year}/cost_reports_hha.parquet
  - staging.stg_cms__cost_reports_hha (PostgreSQL)
"""
...
def validate_hha_cr(df: pd.DataFrame) -> ValidationReport:
    report = ValidationReport(source="cost_reports_hha")
    check_required_columns(df, ["rpt_rec_num", "ccn"], report)
    check_column_not_null(df, "rpt_rec_num", report, severity="BLOCK")
    check_row_count(df, min_rows=3_000, max_rows=15_000, report=report, severity="WARN")
    return report
...
def run(
    ...
    log.info("cost_reports_hha_start", run_date=str(run_date), data_year=data_year)

    if source_path:
        landing = source_path if source_path.is_dir() else source_path.parent
    else:
        source_def = get_source("cost_reports_hha")
        landing = resolve_landing_path("cost_reports_hha", run_date, data_year)
    ...
    rows = copy_dataframe_to_pg(rpt_df[out_cols], "stg_cms__cost_reports_hha", "staging", if_exists="append")
    results["stg_cost_reports_hha"] = rows

    log.info("cost_reports_hha_complete", **results)
    return results
